...
/// to %Pictures%\SnipAgent if the configured save folder is unusable (deleted,
...
public static class ImageSaveService
{
    ...
    /// %Pictures%\SnipAgent (creating it if necessary) and reports why the fallback
    ...
    private static bool TryEnsureWritableFolder(string folder, out string? failureReason)
    {
        try
        {
            ...
            var probePath = Path.Combine(folder, $".snipagent_write_probe_{Guid.NewGuid():N}.tmp");
